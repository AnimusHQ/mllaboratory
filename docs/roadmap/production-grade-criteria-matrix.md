...
| AC‑03 Полный аудит и экспорт | AuditEvent append‑only, outbox‑экспорт NDJSON/SIEM, события экспорта, аудит promotion/export модели, аудит исходящих webhook‑доставок | `audit.yaml`, `experiments.yaml` | `scripts/go_test.sh ./closed/...`, `make integrations-test`, audit export tests | `docs/enterprise/08-security-model.md`, `docs/enterprise/09-operations-and-reliability.md`, `docs/contracts/index.md`, `docs/ops/webhooks.md` |
...
| AC‑08 Метрики/логи/трейсы CP+DP | базовые `/metrics`, структурные логи, OTel‑переменные окружения, метрики webhook‑доставок | `dataplane_internal.yaml`, сервисные health endpoints | smoke‑проверка `/metrics` и readiness | `docs/ops/observability.md`, `docs/ops/webhooks.md`, `docs/roadmap/execution-backlog.md` |
