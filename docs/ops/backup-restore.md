...
## Частичное восстановление (симуляция)
...
- Ожидаемое поведение: сервисы поднимаются, но запросы к отсутствующим объектам возвращают `404/410` без паники и без нарушения целостности.

## Восстановление из устаревшего бэкапа
Используется для проверки миграций и совместимости.

- Восстановить бэкап, отстающий на 1–2 релиза.
- Применить миграции до актуальной версии.
- Проверить совместимость API, доступность run/spec/ledger артефактов.
